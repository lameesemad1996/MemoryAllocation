...
Memory fillOldProcess(list<Segment> holesSegList, long memSize, bool &invalidTotalMemSizeFlag)
{
	...
	for (it = holesSegList.begin(); it != holesSegList.end(); ++it)
	{
		...
		else
		{
			...
			if (it->getBase() != lastAllocAdd)
			{
				Segment newOldProSeg("Old Process", lastAllocAdd, it->getBase() - lastAllocAdd, Segment::oldProcess);
				newOldProSeg.setID(ID);
				returnable.memorySegmentsList.push_back(newOldProSeg);
				it->setID(ID + 1);
				returnable.memorySegmentsList.push_back(*it);
...
for (int noOfHole = 0; noOfHole < inputHoleNo; noOfHole++)
{
	...
	if (noOfHole == 0)
	{
		...
		if (holebase != 0)
		{
			Segment newOldProSeg("Old Process", 0, holebase - 1, Segment::oldProcess);
			lastAllocAdd = holebase + holelimit;
			myMem.memorySegmentsList.push_back(newOldProSeg);
			myMem.memorySegmentsList.push_back(newseg);
		}
		else
		{
			myMem.memorySegmentsList.push_back(newseg);
			lastAllocAdd = holelimit;
		}
	}
	else
	{
		if (holebase != lastAllocAdd)
		{
			Segment newOldProSeg("Old Process", lastAllocAdd, holebase - lastAllocAdd, Segment::oldProcess);
			myMem.memorySegmentsList.push_back(newOldProSeg);
			myMem.memorySegmentsList.push_back(newseg);
			lastAllocAdd = holebase + holelimit;
		}
...
int main()
{
	while (1)
	{
		...
		for (int noOfPro = 0; noOfPro < inputProNumber; noOfPro++)
		{
			...
			bool result = myMem.first_fit_allocate_check(newProcess);
			bool result2 = myMem.first_fit_allocate(newProcess);
